		...
		public static function search_formatted($dataList,$arr_favorites = array(),$arr_castingcart = array(), $casting_availability = ''){
			...
			if(is_user_logged_in() && strpos($type,"profilecastingcart") <= -1){
				...
				if($rb_agency_option_profilelist_favorite){
					...
					$displayActions .= "<div id=\"profile-favorite\" class=\"favorite\"><a href=\"javascript:;\" title=\"".(in_array($dataList["ProfileID"], $arr_favorites)?"Remove from Favorites":"Add to Favorites")."\" attr-id=\"".$dataList["ProfileID"]."\" class=\"".(in_array($dataList["ProfileID"], $arr_favorites)?"active":"inactive")." favorite\"><strong>&#9829;</strong>&nbsp;".(in_array($dataList["ProfileID"], $arr_favorites)?"Remove from Favorites":"Add to Favorites")."</a></div>";
				}
			...
			if ($p_image){
				if(get_query_var('target')!="print" AND get_query_var('target')!="pdf"){
					...
					$displayHTML .="<div  class=\"image\">"."<a href=\"". RBAGENCY_PROFILEDIR ."". $dataList["ProfileGallery"] ."/\" style=\"background-image: url()\"><img src='". get_bloginfo("url")."/wp-content/plugins/rb-agency/ext/timthumb.php?src=".RBAGENCY_UPLOADDIR . $dataList["ProfileGallery"] ."/". $p_image ."&w=180&h=230'/></a>".$images."</div>\n";
				} else {
					$displayHTML .="<div  class=\"image\">"."<a href=\"". RBAGENCY_PROFILEDIR ."". $dataList["ProfileGallery"] ."/\" style=\"background-image: url()\"><img src='". get_bloginfo("url")."/wp-content/plugins/rb-agency/ext/timthumb.php?src=".RBAGENCY_UPLOADDIR . $dataList["ProfileGallery"] ."/". $p_image ."&w=180&h=230'/></a>".$images."</div>\n";
				}
			} else {
			...
			if ($rb_agency_option_profilelist_expanddetails) {
				...
				if($dataList["ProfileLocationState"]!="" && $rb_agency_option_detail_state  == 1){
					$displayHTML .= "<span class=\"divider\">".(rb_agency_get_age($dataList["ProfileDateBirth"])>0?",":" ")."</span><span class=\"details-state\">". rb_agency_getStateTitle($dataList["ProfileLocationState"],true) ."</span>";
				}
				$displayHTML .= "</div>\n";
			}
