...
class RBAgency_Update {
	...
	*/
	...
	public function getRemote_version() {
		$request = wp_remote_post($this->update_path, array('method'=>'POST','body' => array('action' => 'version')));
	...
	public function getRemote_information() {
		$request = wp_remote_post($this->update_path, array('method'=>'POST','body' => array('action' => 'info')));
		if (!is_wp_error($request) || wp_remote_retrieve_response_code($request) == 200) {
	...
	public function getRemote_license() {
		$request = wp_remote_post($this->update_path, array('method'=>'POST','body' => array('action' => 'license')));
		if (!is_wp_error($request) || wp_remote_retrieve_response_code($request) === 200) {
			return $request['body'];
		}
		return false;
