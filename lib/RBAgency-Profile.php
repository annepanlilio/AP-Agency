...
class RBAgency_Profile {
	 ...
	 */
		...
		public static function search_formatted($dataList,$arr_favorites = array(),$arr_castingcart = array(), $casting_availability = '',$plain = false){
			 ...
			 */
			$displayHTML ="";			
			$displayHTML .= "<div data-profileid=\"".$dataList["ProfileID"]."\" id=\"rbprofile-".$dataList["ProfileID"]."\" class=\"rbprofile-list profile-list-layout0 profilelist-style-".$rb_agency_option_layoutprofilelist."\" >\n";
			if(!$plain){
			...
			if ($p_image){
				if(get_query_var('target')!="print" AND get_query_var('target')!="pdf"){
					...
					$displayHTML .="<div  class=\"image\">"."<a href=\"". RBAGENCY_PROFILEDIR ."". $dataList["ProfileGallery"] ."/\" style=\"background-image: url('". get_bloginfo("url")."/wp-content/plugins/rb-agency/ext/timthumb.php?src=".RBAGENCY_UPLOADDIR . $dataList["ProfileGallery"] ."/". $p_image ."&w=180&h=230&a=t')\"></a>".$images."</div>\n";
				} else {
					$displayHTML .="<div  class=\"image\">"."<a href=\"". RBAGENCY_PROFILEDIR ."". $dataList["ProfileGallery"] ."/\" style=\"background-image: url('". get_bloginfo("url")."/wp-content/plugins/rb-agency/ext/timthumb.php?src=".RBAGENCY_UPLOADDIR . $dataList["ProfileGallery"] ."/". $p_image ."&w=180&h=230&a=t')\"></a>".$images."</div>\n";
				}
			} else {
