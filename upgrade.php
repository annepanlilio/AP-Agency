 ...
 */
	...
	if (substr(get_option('rb_agency_version'), 0, 7) == "2.0.9.4") {
	...
	if (substr(get_option('rb_agency_version'), 0, 7) == "2.0.9.7") {
		update_option('rb_agency_version', "2.1");
	}
	...
	if (substr(get_option('rb_agency_version'), 0, 3) == "2.4") {
		...
		$wpdb->query("CREATE TABLE IF NOT EXISTS 
					...
					ProfileCustomID 
			FROM wp_agency_customfield_mux WHERE ProfileCustomID = 20 GROUP BY ProfileID)");

		// Delete old checkboxes value from 'wp_agency_customfield_mux'
		$wpdb->query("DELETE FROM wp_agency_customfield_mux WHERE ProfileCustomID = 20");
	...
	if (substr(get_option('rb_agency_version'), 0, 5) == "2.4.8") {
